...
Card* Computer::pickCard(Game::GAME currentGame)
{
  ...
  {
    // ako igramo igru sto vise, zelimo da bacimo najjacu kartu da bismo odneli
    // talon, tj uzeli poene
    QList<Card*>::iterator pos = std::max_element(
        cardsInHand.begin(), cardsInHand.end(), [talonSuit](Card card) {
  ...
  {
    if ( std::find_if(cardsInHand.begin(), cardsInHand.end(),
                      [talonSuit](Card card) {
                        return card.getCardSuit() == talonSuit;
                      }) )
    {
      QList<Card*>::iterator pos = std::min_element(
          cardsInHand.begin(), cardsInHand.end(), [talonSuit](Card card) {
            if ( card.getCardSuit() == talonSuit )
              return card.getCardNumber();
            return static_cast<unsigned int>(Card::Number::ACE) + 1;
    ...
    else
    {
      ...
      //        card playingCard =  std::max_element(cardsInHand.begin(),
      //        cardsInHand.end(), [](card& card)
      //        {
      ...
      unsigned int numHearts = std::count_if(
          cardsInHand.begin(), cardsInHand.end(),
          [](Card* card) { return card->getCardSuit() == Card::Suit::HEART; });
      unsigned int numDiamonds =
          std::count_if(cardsInHand.begin(), cardsInHand.end(), [](Card* card) {
            return card->getCardSuit() == Card::Suit::DIAMOND;
          });
      unsigned int numSpades = std::count_if(
          cardsInHand.begin(), cardsInHand.end(),
          [](Card* card) { return card->getCardSuit() == Card::Suit::SPADE; });
      unsigned int numClubs = std::count_if(
          cardsInHand.begin(), cardsInHand.end(),
          [](Card* card) { return card->getCardSuit() == Card::Suit::CLUB; });

      Card* playingCard = cardsInHand.front();
      // izbacuje najvecu kartu ili ukoliko ih ima vise iste jacine onu koja je
      // "sama"
      for ( auto card : cardsInHand )
      {
        switch ( card->getCardSuit() )
        {
          case Card::Suit::HEART:
            ...
            else if ( playingCard->getCardNumber() == card->getCardNumber() &&
                      numHearts == 1 )
            {
              playingCard = card;
            }
