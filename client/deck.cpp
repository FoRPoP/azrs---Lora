...
Deck::Deck()
{
  ...
  for ( unsigned int s = static_cast<unsigned int>(Card::Suit::CLUB);
        s <= static_cast<unsigned int>(Card::Suit::SPADE); ++s )
  {
    for ( unsigned int num = static_cast<unsigned int>(Card::Number::SEVEN);
          num <= static_cast<unsigned int>(Card::Number::ACE); ++num )
    {
      Card* card =
          new Card(static_cast<Card::Number>(num), static_cast<Card::Suit>(s));
      deck.append(card);
      numberOfCards += 1;
    }
  }
