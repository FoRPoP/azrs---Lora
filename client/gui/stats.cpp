...
int Stats::saveStatsFile()
{
  ...
  for ( int i = 0; i < numOfPlayers; i++ )
  {
    out << firstPlace[i] << " " << secondPlace[i] << " " << thirdPlace[i] << " "
        << fourthPlace[i] << " " << totalScore[i] << " " << bestScore[i] << " "
        << worstScore[i] << '\n';
  }
...
int Stats::loadStatsFile()
{
  ...
  if ( value < 1 )
  {
    file.rename(QDir::homePath() +
                STATS_BACKUP_FILE);  // rename also close the file
    return FCORRUPTED;
  }
  gameStarted = value;
  ...
  while ( !file.atEnd() )
  {
    ...
    for ( int i = 0; i < 7; i++ )
    {
      ...
      {
        case 0:
          firstPlace[cpt] = value;
          break;
        case 1:
          secondPlace[cpt] = value;
          break;
        case 2:
          thirdPlace[cpt] = value;
          break;
        case 3:
          fourthPlace[cpt] = value;
          break;
        case 4:
          totalScore[cpt] = value;
          break;
        case 5:
          bestScore[cpt] = value;
          break;
        case 6:
          worstScore[cpt] = value;
          break;
      }
    }
    cpt++;
...
{
  double total =
      firstPlace[plr] + secondPlace[plr] + thirdPlace[plr] + fourthPlace[plr];
  ...
  switch ( stats )
  {
    ...
    case STATS_GAME_STARTED:
      ui->label_4->setText(QString::number(gameStarted));
    case STATS_GAME_FINISHED:
      ui->label_5->setText(
          QString::number(gameFinished) + " (" +
          QString::number(double(gameFinished * 100 / started), 'f', 1) +
          QString("%)"));
      break;
    ...
    case STATS_FOURTH_PLACE:
      ...
      item_firstPlace[plr]->setText(
          QString::number(value) + " (" +
          QString::number(double(value * 100 / total), 'f', 1) + "%)");
      value = secondPlace[plr];
      item_secondPlace[plr]->setText(
          QString::number(value) + " (" +
          QString::number(double(value * 100 / total), 'f', 1) + "%)");
      value = thirdPlace[plr];
      item_thirdPlace[plr]->setText(
          QString::number(value) + " (" +
          QString::number(double(value * 100 / total), 'f', 1) + "%)");
      value = fourthPlace[plr];
      item_fourthPlace[plr]->setText(
          QString::number(value) + " (" +
          QString::number(double(value * 100 / total), 'f', 1) + "%)");
      break;
  }
...
void Stats::createWindow()
{
  ...
  for ( int i = 0; i < numOfPlayers; i++ )
  {
    ...
    double total =
        firstPlace[i] + secondPlace[i] + thirdPlace[i] + fourthPlace[i];
    ...
    for ( int i2 = 0; i2 < 4; i2++ )
    {
      ...
      switch ( i2 )
      {
        case 0:
          value = firstPlace[i];
          item_firstPlace[i] = new QTableWidgetItem(
              QString::number(value) + " (" +
              QString::number(double(value * 100 / total), 'f', 1) + "%)");
          item_firstPlace[i]->setFlags(item_firstPlace[i]->flags() &
                                       ~Qt::ItemIsEditable);
        ...
        case 1:
          value = secondPlace[i];
          item_secondPlace.append(new QTableWidgetItem(
              QString::number(value) + " (" +
              QString::number((value * 100 / total), 'f', 1) + "%)"));
          item_secondPlace[i]->setFlags(item_secondPlace[i]->flags() &
                                        ~Qt::ItemIsEditable);
        ...
        case 2:
          value = thirdPlace[i];
          item_thirdPlace.append(new QTableWidgetItem(
              QString::number(value) + " (" +
              QString::number(double(value * 100 / total), 'f', 1) + "%)"));
          item_thirdPlace[i]->setFlags(item_thirdPlace[i]->flags() &
                                       ~Qt::ItemIsEditable);
        ...
        case 3:
          value = fourthPlace[i];
          item_fourthPlace.append(new QTableWidgetItem(
              QString::number(value) + " (" +
              QString::number(double(value * 100 / total), 'f', 1) + "%)"));
          item_fourthPlace[i]->setFlags(item_fourthPlace[i]->flags() &
                                        ~Qt::ItemIsEditable);
    ...
    item_bestScore[i]->setFlags(item_bestScore[i]->flags() &
                                ~Qt::ItemIsEditable);
    item_worstScore[i]->setFlags(item_worstScore[i]->flags() &
                                 ~Qt::ItemIsEditable);
