...
void MainWindow::initializeGraphics() {
   imageEmptyCard = new QImage("../../resources/cards/1x/empty.png", "PNG");
    ...
    for(int i=0; i<=11;++i){
        ...
        labels[i]->show();
        labels[i]->setPixmap(QPixmap::fromImage(deck->getCardBackImage()->scaledToHeight(cardHeight)));
...
//  stats->increaseStats(lora->  getPlayerAtPos(0)->getPlayerName(), STATS_FIRST_PLACE);
//  stats->increaseStats(lora->  getPlayerAtPos(1)->getPlayerName(), STATS_SECOND_PLACE);
//  stats->increaseStats(lora->  getPlayerAtPos(2)->getPlayerName(), STATS_THIRD_PLACE);
//  stats->increaseStats(lora->  getPlayerAtPos(3)->getPlayerName(), STATS_FOURTH_PLACE);


// stats->setScore(lora->  getPlayerAtPos(0)->getPlayerName(), lora->  getPlayerAtPos(0)->getPoints());
// stats->setScore(lora->  getPlayerAtPos(1)->getPlayerName(), lora->  getPlayerAtPos(1)->getPoints());
// stats->setScore(lora->  getPlayerAtPos(2)->getPlayerName(), lora->  getPlayerAtPos(2)->getPoints());
// stats->setScore(lora->  getPlayerAtPos(3)->getPlayerName(), lora->  getPlayerAtPos(3)->getPoints());


  message("Player "+ lora->getPlayerAtPos(0)->getPlayerName()+" won 1st place");
  message("Player "+ lora->getPlayerAtPos(1)->getPlayerName()+" won 2nd place");
  message("Player "+ lora->getPlayerAtPos(2)->getPlayerName()+" won 3rd place");
  message("Player "+ lora->getPlayerAtPos(3)->getPlayerName()+" won 4th place");
    ...
    if(lora->getPlayerAtPos(0) == player){
        ...
        for(int i=0; i<numOfCards; ++i){
            ...
            if(isCardDisabled(cards[i])){
                labels[i]->setPixmap(QPixmap::fromImage(cards[i]->getCardImageDisabled()->scaledToHeight(cardHeight)));
                labels[i]->setDisabled(true);
            }
            else{
                labels[i]->setPixmap(QPixmap::fromImage(cards[i]->getCardImage()->scaledToHeight(cardHeight)));
...
void MainWindow::showYourTurn() {
  ...
  if(player != lora->getPlayerAtPos(0))
      return;
  showDeck();
  labels[8 + player->getUserId()]->setPixmap(QPixmap::fromImage(imageYourTurn->scaledToHeight(cardHeight)));
...
void MainWindow::setLanguage(int lang) {
 ...
 switch (lang) {
    case LANG_ENGLISH: if (translator.load(QLocale(QLocale::English), QLatin1String("translation"), QLatin1String("_"), QLatin1String(":/languages"))) {
     ...
     break;
case LANG_SERBIAN:  if (translator.load(QLocale(QLocale::Serbian), QLatin1String("translation"), QLatin1String("_"), QLatin1String(":/languages"))) {
       qApp->installTranslator(&translator);
...
void MainWindow::addCardToTable(Card* card, Player* plr){
    labels[14+plr->getUserId()]->setPixmap(QPixmap::fromImage(card->getCardImage()->scaledToHeight(cardHeight)));
