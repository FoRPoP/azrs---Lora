...
void Game::gameHeart(QTcpSocket* con)
{
  for ( int i = 0; i < 8; ++i )
  {
    ...
    while ( !talon->empty() )
    {
      ...
      if ( played.first->getCardSuit() == suit &&
           played.first->getCardNumber() > strongestCard )
      {
...
void Game::gameJackClubLastHand(QTcpSocket* con)
{
  for ( int i = 0; i < 8; ++i )
  {
    if ( con == nullptr )
    {
      ...
      {
        Card* selectedCard =
            players[j]->pickCard(GAME::JACK_CLUB_LAST_HAND, mainWindow);
    ...
    {
      onlineRound(con, m_clientPlayerNumber, players,
                  GAME::JACK_CLUB_LAST_HAND);
    }
    ...
    while ( !talon->empty() )
    {
      ...
      if ( played.first->getCardSuit() == Card::Suit::CLUB &&
           played.first->getCardNumber() ==
               static_cast<unsigned int>(Card::Number::JACK) )
      {
        jackClub = true;
      }
    }
...
void Game::gameKingHeartLastHand(QTcpSocket* con)
{
  for ( int i = 0; i < 8; ++i )
  {
    if ( con == nullptr )
    {
      ...
      {
        Card* selectedCard =
            players[j]->pickCard(GAME::KING_HEART_LAST_HAND, mainWindow);
    ...
    while ( !talon->empty() )
    {
      ...
      if ( played.first->getCardSuit() == Card::Suit::HEART &&
           played.first->getCardNumber() ==
               static_cast<unsigned int>(Card::Number::KING) )
      {
        kingHeart = true;
      }
    }
...
void Game::gameLora(QTcpSocket* con)
{
  ...
  //          QVector<Talon> taloni{talonHeart, talonSpade, talonDiamond,
  //          talonClub};
  ...
  //                  if (checkLora()){
  ////                      std::cout << "Player " <<
  ///players.front().getPlayerName() << " bira kartu." << std::endl;
  //                      unsigned selectedCard;
  ////                      std::cin >> selectedCard;
  //                      Card::Suit selectedCardSuit =
  //                      players.front().getCardsInHand()[selectedCard].getTalonSuit();
  //                      unsigned int selectedCardNumber =
  //                      players.front().getCardsInHand()[selectedCard].getCardNumber();

  //                      bool validCard = false;
  //                      while (!validCard){
  //                          for (unsigned int brojTalona = 0; brojTalona < 4;
  //                          ++brojTalona){
  //                              if (!taloni[brojTalona].empty() &&
  //                              taloni[brojTalona].getTalonSuit() ==
  //                              selectedCardSuit){
  //                                  if (taloni[brojTalona].getCardNumber() ==
  //                                  static_cast<unsigned
  //                                  int>(Card::Number::KING) &&
  //                                  selectedCardNumber == static_cast<unsigned
  //                                  int>(Card::Number::SEVEN)){
  //                                      taloni[brojTalona].addCard(&players.front().getCardsInHand()[selectedCard],
  //                                      &players[j]); validCard = true; break;
  //                                  }
  ...
  //                              case Card::Suit::TREF:
  //                                  if (talonClub.empty()){
  //                                      talonClub.addCard(&players.front().getCardsInHand()[selectedCard],
  //                                      &players[j]); validCard = true; break;
  //                                  }

  //                              case Card::Suit::PIK:
  //                                  if (talonSpade.empty()){
  //                                      talonSpade.addCard(&players.front().getCardsInHand()[selectedCard],
  //                                      &players[j]); validCard = true; break;
  //                                  }

  //                              case Card::Suit::HERC:
  //                                  if (talonHeart.empty()){
  //                                      talonHeart.addCard(&players.front().getCardsInHand()[selectedCard],
  //                                      &players[j]); validCard = true; break;
  //                                  }

  //                              case Card::Suit::KARO:
  //                                  if (talonDiamond.empty()){
  //                                      talonDiamond.addCard(&players.front().getCardsInHand()[selectedCard],
  //                                      &players[j]); validCard = true; break;
  //                                  }
  //                          }

  //                          if (!validCard){
  //                              std::cout << "Izabrali ste nekorektnu kartu!
  //                              Izaberite drugu." << std::endl; std::cin >>
  //                              selectedCard; selectedCardSuit =
  //                              players.front().getCardsInHand()[selectedCard].getTalonSuit();
  //                              selectedCardNumber =
  //                              players.front().getCardsInHand()[selectedCard].getCardNumber();
  //                          }
...
void Game::play(QTcpSocket* con)
{
  ...
  if ( con != nullptr )
  {
    ...
    {
      QString pom;
      for ( int j = i * (perm_length + 1);
            j < i * (perm_length + 1) + perm_length; j++ )
        pom += deckMessage[j];
...
void Game::onlineRound(QTcpSocket* con,
                       int playerPosition,
